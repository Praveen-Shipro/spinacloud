...
export default function Testimonials() {
            ...
            {testimonials.map((t, idx) => {
              ...
              return (
                <div 
                  ...
                  className={`group relative p-8 rounded-2xl bg-white/[0.02] backdrop-blur-xl transition-all duration-500 ease-[cubic-bezier(0.25,1,0.5,1)] will-change-transform flex flex-col justify-between shadow-lg shadow-black/20 ${
                    isHovered
                      ? 'scale-105 md:scale-[1.06] -translate-y-2 shadow-2xl shadow-orange-500/10 border border-orange-500/50 z-10 bg-white/[0.05]'
                      : isOther
                      ? 'scale-[0.95] md:scale-[0.96] opacity-70 border border-white/5'
  ...
  return (
    <section className="px-4 relative bg-[linear-gradient(to_right,#FF6600_55%,#FF9752_100%)]">
      <div className="container mx-auto max-w-6xl 2xl:max-w-7xl py-24 border-b border-gray-300/40">
        <div className="text-center mb-16">
          <h2 className="text-black/80 font-semibold tracking-wider text-sm uppercase mb-4">Testimonials Section</h2>
          <h3 className="text-xl md:text-2xl font-bold font-montserrat text-white mx-auto">
          ...
          {testimonials.map((t, idx) => {
            ...
            return (
              <div 
                key={idx}
                onMouseEnter={() => setHoveredIdx(idx)}
                className={`bg-black border rounded-2xl p-8 flex flex-col justify-between transition-all duration-500 ease-[cubic-bezier(0.25,1,0.5,1)] will-change-transform ${
                  isHovered
                    ? 'scale-105 md:scale-[1.06] -translate-y-2 shadow-2xl shadow-black/70 border-white/40 z-10'
                    : isOther
                    ? 'scale-[0.95] md:scale-[0.96] opacity-75 border-white/5'
                    : 'scale-100 opacity-100 border-white/10'
                }`}
              >
                <div>
